...
namespace far_memory
{
  ...
  FarMemManager::RegionManager::RegionManager(uint64_t size, bool is_local)
  {
    ...
    {
      LOG_PRINTF("%s Free regions %f. Req %d\n",
                 "Error: two few available regions.", free_regions_count,
                 2 * helpers::kNumSocket1CPUs);
      exit(-ENOSPC);
    }
  ...
  void FarMemManager::swap_out(GenericFarMemPtr *ptr, Object obj)
  {
    ...
    {
      if (evac_notifier(obj, write_object_fn))
      { // Ptr removed.
        return;
      }
    }
