...
// constexpr uint64_t kCacheSize = (51ULL << 20);
...
void gen_zero_array(uint64_t num_entries, uint64_t *raw_array)
{
  for (uint64_t i = 0; i < num_entries; i++)
  {
    raw_array[i] = 0;
...
void do_work(FarMemManager *manager)
{
  ...
  gen_zero_array(kNumEntries, raw_array_A);
  gen_zero_array(kNumEntries, raw_array_B);
  ...
  for (uint64_t i = 0; i < kNumEntries; i++)
  {
    ...
    {
      cout << array_C.at(scope, i) << " != "
           << raw_array_A[i] << " + " << raw_array_B[i] << endl;
      goto fail;
    }
  }
